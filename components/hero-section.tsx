...
export function HeroSection() {
  ...
  useEffect(() => {
    if (!mounted || !scrollRef2.current) return
  ...
  useEffect(() => {
    if (scrollRef2.current) {
      scrollRef2.current.scrollLeft = Math.abs(scrollPosition2)
    }
  }, [scrollPosition2])
    ...
    <section className="bg-gradient-to-b from-card/50 to-background py-3 md:py-4">
      <div className="container mx-auto px-3">
        ...
        <div className="relative overflow-hidden mb-2">
          <div ref={scrollRef} className="flex gap-2 overflow-hidden" style={{ scrollBehavior: "auto" }}>
            {duplicatedCategories.map((cat, index) => (
              <Link
                key={`${cat.id}-${index}`}
                href={`/kategorije/${cat.id}`}
                className="group relative flex shrink-0 items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-medium text-white transition-all duration-300 md:px-4 md:py-2 md:text-xs"
                style={{
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.boxShadow = `0 0 25px ${cat.glowColor}90, 0 0 50px ${cat.glowColor}50`
                  e.currentTarget.style.transform = "scale(1.05)"
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.boxShadow = `0 0 15px ${cat.glowColor}60, 0 0 30px ${cat.glowColor}30`
        ...
        <div className="relative overflow-hidden">
          <div ref={scrollRef2} className="flex gap-2 overflow-hidden" style={{ scrollBehavior: "auto" }}>
            {duplicatedCategories.map((cat, index) => (
              <Link
                key={`${cat.id}-reverse-${index}`}
                href={`/kategorije/${cat.id}`}
                className="group relative flex shrink-0 items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-medium text-white transition-all duration-300 md:px-4 md:py-2 md:text-xs"
                style={{
                  background: `linear-gradient(135deg, ${cat.glowColor}90, ${cat.glowColor}60)`,
