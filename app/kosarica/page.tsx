    ...
    try {
      ...
      if (res.ok) {
        ...
        if (apiCart.length === 0) {
          ...
          setCart(apiCart)
          saveCartToLocal(apiCart) // Keep localStorage in sync
        }
      } else {
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            <div className="space-y-4 lg:col-span-2">
              {cart.map((item) => (
                <div
                  key={item.productId}
                  className="flex flex-col gap-4 rounded-xl border border-border bg-card p-4 md:flex-row md:p-6"
                >
                  <div className="h-32 w-full flex-shrink-0 overflow-hidden rounded-lg bg-muted md:w-32">
                    <img
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mb-4 text-xl font-bold text-primary">{formatPrice(item.price)}</p>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => updateQuantity(item.productId, item.quantity - 1)}
                          className="flex h-10 w-10 items-center justify-center rounded-lg border border-border bg-background transition-colors hover:border-primary"
                        >
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 rounded-xl border border-border bg-card p-6">
                <h2 className="mb-6 text-xl font-bold text-foreground">Sazetak narudzbe</h2>
                ...
                <Link
                  ...
                  className="mb-6 block w-full rounded-lg bg-gradient-to-r from-primary to-primary/80 py-4 text-center font-bold text-primary-foreground transition-all hover:from-primary/90 hover:to-primary/70"
                >
                  Nastavi na placanje
